...
const ValidateHeaderFields = (payload) =>{
    ...
    if (payload) {
        ...
        if (vldtInvoiceNumber(invoiceNumber) === false) {
            errors.push(`Invoice N0: Should be more than 4 chars || no spaces allowed betweeen`);
        }
        // Total Levy
        ...
        // Reference
        if (vldtReference(reference) === false) {
            errors.push( `Reference incorrect`);
        }
        // Group Reference
        if (vldtGroupReferenceId(groupReferenceId) === false) {
            errors.push(`Group Reference: Can only be Numeric`);
        }
        // Purchase Order Reference
        if (vldtPurchaseOrderReference(purchaseOrderReference) === false) {
            errors.push(`POR Can only be Numeric`);
        }
    }
    else {
